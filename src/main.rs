...
#[tokio::main]
async fn main() {
    let db_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://./data/co2.db".into());
    ...
    let app = Router::new()
        ...
        .route(
            "/settings/regen-topic",
            post(handlers::settings_regen_topic),
        )
